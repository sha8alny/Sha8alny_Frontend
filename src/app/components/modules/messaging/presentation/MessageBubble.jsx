...
function getFileTypeInfo(file) {
    ...
    if (typeof file === 'string') {
        ...
        fileType = file.type
        extension = fileName.split(".").pop()?.toLowerCase()
        url = fileType.startsWith('image/') || fileType.startsWith('video/')
            ? URL.createObjectURL(file)
            : null
    } else if (file && file.content && (file.content instanceof File || file.content.name)) {
        return getFileTypeInfo(file.content)
    } else if (file && file.url) {
        return getFileTypeInfo(file.url)
    } else {
        return { fileName: "File", fileType: "", extension: "", url: null, icon: <DescriptionIcon className="h-4 w-4" /> }
    }
                ...
                <div>
                            ...
                            <div className="space-y-2">
                                {message.messageContent.map((item, index) => {
                                    if (item.type === 'text') {
                                        ...
                                        return <MediaRenderer key={index} media={item.content || item} index={index} />
                                    }
                                    return null
                                })}
                            </div>
                    ...
                    <div className={`flex items-center text-xs text-muted-foreground mt-1 ${isCurrentUser ? "justify-end" : ""}`}>
                        ...
                        {isCurrentUser && message.read && (
                            <DoneAllIcon className="ml-1" sx={{ fontSize: 20, color: green[400] }} />
                        )}
                    </div>
                </div>
            </div>
