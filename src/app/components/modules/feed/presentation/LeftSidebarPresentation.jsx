    ...
    <aside className="w-full flex flex-col gap-2">
      {/* Profile Card Skeleton */}
        ...
        <div className="px-4 pt-1 flex flex-col pb-4">
          <div className={`${isLoading && "animate-pulse"} h-6 w-32 bg-primary/60 rounded-2xl mb-2`}/>
          <div className={`${isLoading && "animate-pulse"} mb-2 bg-primary/60 w-36 h-4 rounded-2xl`}/>
          <div className={`${isLoading && "animate-pulse"} w-60 h-3 rounded-2xl bg-primary/60`}/>
        </div>
          </Container>
      ...
      <Container className="p-4 w-full border shadow-sm flex flex-col gap-2">
        {[...Array(3)].map((_, index) => (
          <div key={index} className="flex gap-2 items-center">
            <div className={`${isLoading && "animate-pulse"} bg-primary/60 h-4 w-3/4 rounded-2xl`} />
          </div>
        ))}
      </Container>
      ...
      <Container className="flex flex-col w-full border shadow-sm p-4">
        <div className={`${isLoading && "animate-pulse"} bg-primary/60 h-6 w-3/4 rounded-2xl mb-4`}></div>
        {[...Array(4)].map((_, index) => (
          <div key={index} className="flex items-center mb-2">
            <div className={`${isLoading && "animate-pulse"} bg-primary/60 h-5 w-[90%] rounded-2xl mr-2`}/>
          </div>
        ))}
      </Container>
    </aside>
