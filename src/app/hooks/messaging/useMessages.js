...
export function useMessages(selectedConversation, currentUser) {
  ...
  const lastSubscriptionParams = useRef({
    conversationId: null,
    currentUser: null,
  });
  ...
  const handleSendMessage = useCallback(
    (receiverName, messageText, media) => {
      if (
        (!messageText.trim() && (!media || media.length === 0)) ||
        !currentUser
      )
        return;
    ...
    (username, conversationId, isTyping) => {
      ...
      const debouncedFn = debounce(() => {
        ...
        if (debouncedUpdatePendingRef.current) {
          messagingService
            .updateTypingStatus(username, conversationId, isTyping)
            .catch((err) =>
  ...
  useEffect(() => {
    ...
    if (
      ...
      lastSubscriptionParams.current.conversationId &&
      currentUser
    ) {
      clearTypingStatus(
        currentUser,
    ...
    messagesSubscriptionRef.current =
      messagingService.subscribeToConversationMessages(
        conversationId,
        currentUser,
        (newMessages) => {
  ...
  useEffect(() => {
    return () => {
      ...
      if (isTypingRef.current && currentUser && selectedConversation?.id) {
        messagingService
          .updateTypingStatus(currentUser, selectedConversation.id, false)
          .catch((err) =>
